...
class KorgunFinanceAdapter:
    ...
    def fetch_cari_hareketleri(self, location: str, cari_kod: str) -> Dict[str, Any]:
        """
        Cari hareketleri — READ-ONLY normalized ledger (kg_fn @KC_Temp rebuild).

        P0: FaturaTutar/kg_ifn_FaturaTutar, C_Fis cbpg+CFisTutar, Banka FisTip+BankaTutar,
        consolidated finance scope, çek ayrı bilgi paneli (double-count yok).
        INSERT/UPDATE/DELETE YOK.
        """
        loc = (location or '').strip().upper()
        ck = (cari_kod or '').strip()
