...
def login_kullanici(kadi, sifre):
    ...
    if u:
        d = dict(u)
        d['Tip'] = d.get('Tip') or 'sistem'  # AUTH_TIP_FIX_v1: DB.Tip oncelikli
        return d
...
def is_superadmin(user_dict):
    """
    Anayasa Bolum 1.5 - SuperAdmin tek dogruluk kaynagi.
    SADECE su iki durumda True:
      1) KullaniciAdi == 'admin'
      2) Tip == 'sistem' AND RolAd == 'Yönetim'
    Diger kombinasyonlar (usta, personel, hibrit): False
    """
    if not user_dict:
        return False
    if user_dict.get('KullaniciAdi') == 'admin':
        return True
...
def kullanici_yetkileri(user_dict):
    ...
    rows = q("""
        SELECT y.Kod, ry.Gorebilir, ry.Duzenleyebilir,
               ry.can_view, ry.can_create, ry.can_update, ry.can_delete,
    ...
    for r in rows:
        kod = r['Kod']
        # Eski format (geriye uyum - DOKUNULMAZ)
        if r['Gorebilir']:
            yetkiler.add(kod + '.goruntule')
        if r['Duzenleyebilir']:
            yetkiler.add(kod + '.duzenle')
        # Yeni format (V2 7 boyut - D3 te aktif okunacak)
...
def yetki_var(kod, action='can_view'):
    ...
    if action == 'can_view':
        if (kod + '.goruntule') in yk:
            return True
    # 4) Eski fallback - SADECE can_update icin .duzenle (D3.2 - 16.05.2026)
...
def yetki_gerekli(kod, action='can_view'):
    def deco(f):
        @wraps(f)
...
def _tip_guard():
    ...
    # 6) Usta: /hedef/* + /uretim/*
    if tip == 'usta':
        # FAZ UI BUGFIX: USTA_TASKS_GUARD
        _USTA_OK = ('/hedef/', '/uretim/', '/tasks', '/api/tasks', '/personel-giris/')
        if not any(path.startswith(p) for p in _USTA_OK):
