...
def _doviz_kur_bul(metin):
    """
    PDF metninde doviz tutari ve kur bulur.

    1) Mevcut (ardisik) pattern: "USD 71.500,00 36,52150"
    2) PATCH 3.1 fallback: PDF parcalanmasi durumunda
       ("20 U sl S im D 3.875,00 44,51070 11" gibi),
       sadece kur formati (30-50 araligi 4-5 ondalik) aranir.
       Para birimi parcalanmis halde (USD -> U S D) tespit edilmeye calisilir,
       bulunamazsa default USD assume edilir.

    Donus: (doviz_tutari, para_birimi, kur)
    """
    if not metin:
        return None, None, None

    # 1) ARDISIK PATTERN (mevcut davranis)
    m = re.search(
    ...
    if m:
        ...
        if kur is not None and 0.001 < kur < 1000:
            return tutar, para, kur

    # 2) PATCH 3.1 FALLBACK - sadece kur ara (PDF parcalanmasi tolerans)
    m_kur = re.search(r'\b([3-5]\d[,\.]\d{4,5})\b', metin)
    if not m_kur:
        return None, None, None
    kur = _tl_float(m_kur.group(1))
    if kur is None or not (20 < kur < 100):
        return None, None, None

    # Para birimini parcalanmis halde ara - USD harfleri "U S D" gibi olabilir
    para = None
    for p in ('USD', 'EUR', 'GBP', 'CNY', 'JPY'):
        pat = r'\b' + r'\s*'.join(list(p)) + r'\b'
        if re.search(pat, metin):
            para = p
            break
    return None, (para or 'USD'), kur
    ...
    try:
        ...
        for kalem in kalemler:
            ...
            oneriler.append({
                ...
                'alt_kod':             alt_kod,
                'fatura_no':           beyanname_no,
                # PATCH 3: belgeden okunan TCMB kuru kaleme baglanir
                'kur_onerisi':         kur,
            })
