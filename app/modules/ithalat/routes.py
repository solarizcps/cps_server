...
@yetki_gerekli('ithalat.parti', 'can_update')
...
# API - MALIYET
# =====================================================================
@ithalat_bp.route('/api/ithalat/parti/<int:parti_id>/ana-kur', methods=['POST'])
@yetki_gerekli('ithalat.maliyet', 'can_update')
def api_parti_ana_kur(parti_id):
    """PATCH 4: parti.AnaKurTRY guncelle + NULL kalemleri backfill"""
    try:
        data = _json_body() or {}
        ana_kur_try = data.get('ana_kur_try')
        if ana_kur_try is None:
            return _hata('ana_kur_try zorunlu')
        try:
            ana_kur_try = float(ana_kur_try)
        except Exception:
            return _hata('ana_kur_try gecersiz')
        sonuc = ith.ana_kur_guncelle(parti_id, ana_kur_try, kullanici_adi())
        if not sonuc.get('ok'):
            return _hata(sonuc.get('hata', 'Hata'))
        return jsonify({
            'ok': True,
            'ana_kur_try': sonuc.get('ana_kur_try'),
            'guncellenen_kalem': sonuc.get('guncellenen_kalem', 0),
        })
    except Exception as e:
        log.exception('api_parti_ana_kur hata: %s', e)
        return _hata('Istek islenemedi', 500)
...
@yetki_gerekli('ithalat.odeme', 'can_update')
