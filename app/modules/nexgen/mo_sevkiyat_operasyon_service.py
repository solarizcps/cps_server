...
from modules.nexgen.pzm_siparis_read import pzm_siparis_kalemleri_getir, pzm_siparis_ozet
...
def _tahsilat_ozet_siparis(con: sqlite3.Connection, siparis_id: int) -> dict[str, Any]:
    ...
    elif d.get('tahsilat_tarih_kaynagi', '').startswith('GERCEK_SEVK'):
...
def liste_sevkiyat_tab(con: sqlite3.Connection, tab: str) -> list[dict[str, Any]]:
    ...
    for r in rows:
        ...
        d['termin_durum'] = termin_durum_etiket(
            verilen if verilen != '—' else None,
            (d.get('sevk_tarihi') or '')[:10] or None,
            (d.get('teslim_tarihi') or '')[:10] or None,
        )
        out.append(d)
    return out
