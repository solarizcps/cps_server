...
==============================
FAZ-MO-SIPARIS-BACKEND-1 — nexgen_planlama_siparis MO köprü kolonları.
"""
...
def _ensure_columns(con: sqlite3.Connection) -> None:
    ...
    con.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS idx_nps_mo_idempotency
        ON nexgen_planlama_siparis(idempotency_key)
        WHERE idempotency_key IS NOT NULL AND idempotency_key != ''
        """
    )
    con.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_nps_kaynak_modul
        ON nexgen_planlama_siparis(kaynak_modul, durum)
        """
    )
    con.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_nps_mo_gorusme
        ON nexgen_planlama_siparis(mo_gorusme_id)
...
def run(db_path: str | None = None) -> None:
    ...
    log('=' * 70)
    log(f'[{MIGRATION_VERSION}] MO sipariş köprü kolonları')
    ...
    try:
        if _table_exists(con, 'schema_migrations'):
            ...
            if applied and _kolon_var(con, 'nexgen_planlama_siparis', 'kaynak_modul'):
