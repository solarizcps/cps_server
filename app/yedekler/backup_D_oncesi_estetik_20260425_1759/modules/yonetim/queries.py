...
def rol_yetkileri(rol_id):
    """Dict: {yetki_id: {'Gorebilir':0/1, 'Duzenleyebilir':0/1}}"""
    rows = q("""
...
def rol_yetki_kaydet(rol_id, yetki_map, kullanici):
    """
    yetki_map: {yetki_id: {'gor': bool, 'duz': bool}, ...}
    """
    r = rol_tek(rol_id)
    if not r:
        raise ValueError('Rol bulunamadı.')
    ...
    with get_conn() as conn:
        ...
        try:
            ...
            for yid, sec in yetki_map.items():
                ...
                if g or d:
                    cur.execute("""
                        INSERT INTO sistem_rol_yetki (RolId, YetkiId, Gorebilir, Duzenleyebilir)
                        VALUES (?,?,?,?)
                    """, (rol_id, yid, g, d))
                eski_g = eski.get(yid, {}).get('Gorebilir', 0)
                eski_d = eski.get(yid, {}).get('Duzenleyebilir', 0)
                if eski_g != g or eski_d != d:
                    degisen += 1
            cur.execute("COMMIT")
        except Exception:
