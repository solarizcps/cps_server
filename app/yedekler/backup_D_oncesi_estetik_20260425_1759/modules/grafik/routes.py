...
def yetki_gerek(kod):
    """admin veya ilgili yetkiye sahip olmalı."""
    def deco(f):
        @wraps(f)
...
def panel():
    if not session.get('kullanici'):
        return redirect(url_for('auth.login', next=request.path))
    if not _admin_mi() and not (yetki_var('grafik.urun') or yetki_var('grafik.tedarikci')
...
def grafik_belge_sil(belge_id):
    ...
    if not _admin_mi() and not yetki_var(yetki_kod):
        abort(403)
...
@yetki_gerek('grafik.cin_siparis')
def siparis_liste():
    arama = request.args.get('q', '').strip() or None
    durum = request.args.get('d', '').strip() or None
    ted_id = request.args.get('t', '').strip() or None
    ...
    return render_template('grafik/siparis_liste.html',
                           ...
                           arama=arama, durum=durum, ted_id=ted_id, musteri=musteri,
                           fiyat_gor=_admin_mi() or yetki_var('grafik.cin_siparis.fiyat'))
