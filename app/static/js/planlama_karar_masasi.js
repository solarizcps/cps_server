...
(function () {
  ...
  /* FAZ-P1B: Proses zinciri rozetleri */
  function km_render_proses(s) {
    var prosesler = s.prosesler || [];
    if (!prosesler.length) {
      var aktif = s.aktif_proses;
      if (aktif && aktif !== 'BITTI') {
        return '<span class="km-proses-rozet km-proses-rozet-devam">' + _esc(aktif) + '</span>';
      }
      return '<span class="km-hucre-soluk">—</span>';
    }
    var html = '<div class="km-proses-zincir">';
    for (var i = 0; i < prosesler.length; i++) {
      var p = prosesler[i];
      var durum = (p.durum || '').toUpperCase();
      var cls = 'km-proses-rozet ';
      if (durum === 'BITTI')      cls += 'km-proses-rozet-bitti';
      else if (durum === 'DEVAM') cls += 'km-proses-rozet-devam';
      else                        cls += 'km-proses-rozet-bekliyor';
      var ikon = durum === 'BITTI' ? '✓ ' : (durum === 'DEVAM' ? '⟳ ' : '· ');
      html += '<span class="' + cls + '" title="' + _esc(p.ad) + ' · ' + _esc(durum) + '">'
            + ikon + _esc(p.ad || '—') + '</span>';
    }
    html += '</div>';
    if (s.aktif_proses && s.aktif_proses !== 'BITTI') {
      html += '<div class="km-proses-aktif-satir">Aktif: ' + _esc(s.aktif_proses) + '</div>';
    }
    return html;
  }
  ...
  function km_render_satir(s, idx) {
    ...
    return '<tr class="' + rowClass + '" data-satir-id="' + _esc(s.satir_id) + '"' +
      ...
      /* FAZ-P1B: Proses rozet hucresi */
      '<td class="km-proses-hucre">' + km_render_proses(s) + '</td>' +
      ...
      '<td class="km-hucre-merkez km-col-bant">' + bant + '</td>' +
      ...
      '<td class="km-col-talimat">' + km_render_talimat(s) + '</td>' +
  ...
  function km_yukle() {
    ...
    fetch("/planlama/karar-masasi/data", { credentials: "same-origin" })
      ...
      .then(function(data){
        ...
        /* FAZ-P1B: kaynak rozetini güncelle */
        (function(){
          var rozetEl = document.getElementById('km-kaynak-rozet');
          if (rozetEl) {
            if (km_state.kaynak === 'GERCEK') {
              rozetEl.className = 'km-rozet-gercek';
              rozetEl.textContent = 'KORGUN';
            } else {
              rozetEl.className = 'km-rozet-mock';
              rozetEl.textContent = 'MOCK';
            }
          }
        })();
