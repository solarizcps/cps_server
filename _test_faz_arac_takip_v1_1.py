...
with patch('modules.auth.kullanici_yetkileri', return_value=YK), \
    ...
    ok('VEHUI-02 CPS shell preserved', 'atpV2Root' in html and ('sidebar' in html.lower() or 'sn-grup' in html))
    ok('VEHUI-03 no second app shell', html.count('atpV2Root') == 1)
    ...
    ok('VEHUI-09 daily table', 'atpJobsTable' in html and 'atpDailyJobsBody' in html)
    ...
    ok('VEHUI-10 priority badges', 'pri-yuksek' in _css and 'pri-acil' in _css)
    ok('VEHUI-11 status badges', 'Bekliyor' in html or 'Başlangıç' in html)
    wa_r = c.get('/planlama/arac-takip/api/whatsapp?date=2026-09-14&vehicle_id=45077045')
    ok('VEHUI-12 WhatsApp endpoint', wa_r.status_code == 200, str(wa_r.status_code))
    wa = wa_r.get_json()
    ...
    ok('VEHUI-15 route analysis DTO', 'atpRouteExplainerModal' in html and 'Rota Kararı' in html)
