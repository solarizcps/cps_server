...
def _live_canonical() -> Path:
    env = __import__('os').environ.get('CPS_CANONICAL_DB_SOURCE', '').strip()
    return Path(env) if env else Path(r'C:\Solariz_CPS_SERVER\app\mock_data.db')
...
def main() -> int:
    canonical = _live_canonical()
    before = _sha(canonical) if canonical.is_file() else ''
    ...
    after = _sha(canonical) if canonical.is_file() else ''
